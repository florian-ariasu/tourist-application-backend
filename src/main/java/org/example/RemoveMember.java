...
public class RemoveMember extends Command {
  ...
  public void execute(List<String> tokens, PrintWriter printWriter) {
    ...
    Person foundPerson = group.findMember(person);

    if (foundPerson == null) {
      String message = group.getMuseumCode() + " ## " + group.getTimetable() +
        " ## PersonNotExistsException: Person was not found in the group. ## (" + person + ")";
      throw new PersonNotExistsException(message);
    }

    group.removeMember(foundPerson);

    String message = group.getMuseumCode() + " ## " + group.getTimetable() + " ## removed member: " + person;
